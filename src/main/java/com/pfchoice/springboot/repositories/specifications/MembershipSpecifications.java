...
public class MembershipSpecifications implements Specification<Membership> {
	...
	public MembershipSpecifications(String searchTerm, Integer insId, Integer prvdrId, Integer effectiveYear, List<Integer> problemIds) {
		super();
	...
	public Predicate toPredicate(Root<Membership> root, CriteriaQuery<?> cq, CriteriaBuilder cb) {
		...
		if(effectiveYear != null){
			...
			p.getExpressions().add(cb.and(cb.like( root.join("mbrActivityMonthList").get("activityMonth").as(String.class), effectiveYearLikePattern)
					));
		}
