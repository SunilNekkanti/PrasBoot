            ...
            <div class="col-sm-3">
              <div class="form-group col-sm-12">
                <label for="plan">Provider</label>
                 <select class=" form-control" ng-model="ctrl.prvdr" ng-change="ctrl.reset()" ng-options="provider.name for provider in ctrl.prvdrs | filter:{refInsContracts:[{ins:{id:ctrl.insurance.id}}]} | orderBy:'name' track by provider.name"> </select>
              </div>
            </div>
