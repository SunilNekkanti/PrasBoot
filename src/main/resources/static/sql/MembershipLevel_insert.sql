...
insert into membership_level_claim_summary select  distinct :insId,:fileId, :activityMonth, IPA_CD, IPANAME, PCP_PROVIDER_NBR, PCPFIRSTNAME, PCPLASTNAME, CONTRACT_YR, 
...
sum(FUNDING) FUNDING, sum(MEMBER_MONTH_CNT)MEMBER_MONTH_CNT, sum(INST_CLAIMS)INST_CLAIMS, sum(PROF_CLAIMS)PROF_CLAIMS, sum(PHAR_CLAIMS)PHAR_CLAIMS,
 ...
 sum(CAPITATION_VISION)CAPITATION_VISION, sum(IBNR_INST)IBNR_INST, sum(IBNR_PROF)IBNR_PROF,  sum(IBNR_TOT)IBNR_TOT ,  :fileId fileId, now() creadted,now() updated, :username created_by , :username updated_by, 'Y' active
 from csv2table_amg_member_level  ml 
JOIN  ( select c.*,rc.insurance_id,rc.prvdr_id from contract c 
                        JOIN reference_contracts rc on  c.contract_Id = rc.contract_Id  and insurance_id =:insId
