...
insert into provider_adjustment select null,c.insurance_id, c.prvdr_id, :fileId, :activityMonth reportMonth, ml.IPA_CD, ml.RISK_ENTITY_TYPE, ml.CONTRACT_YEAR, ml.TIME_PERIOD, ml.CONTRACT_PERIOD_START_DT, 
...
left join provider_adjustment pa on pa.report_month =:activityMonth and pa.ins_id =c.insurance_id and pa.prvdr_id = c.prvdr_id and pa.ACTIVITYMONTH = ml.ACTIVITYMONTH
...
select a.insurance_id,a.prvdr_id , :activityMonth reportMonth,ACTIVITYMONTH, 
 qmlrfunction(:activityMonth, a.insurance_id,a.prvdr_id,ACTIVITYMONTH,false,false) qmlr,
  amg_funding FUNDING, amg_mbr_cnt MEMBER_MONTH_CNT,  amg_inst  INST_CLAIMS, amg_prof PROF_CLAIMS, amg_phar PHAR_CLAIMS,
   ...
   amg_vision_cap CAPITATION_VISION, amg_ibnr_inst IBNR_INST, amg_ibnr_prof IBNR_PROF,  amg_ibnr_tot IBNR_TOT ,  :fileId fileId, now() creadted,now() updated, :username created_by , :username  updated_by, 'Y' active
    from (
    select    sum(ADJUSTMENT_AMT) ADJUSTMENT_AMT,c.insurance_id,c.prvdr_id,ml.ACTIVITYMONTH
 from csv2table_amg_adjust  ml 
JOIN  ( select c.*,rc.insurance_id,rc.prvdr_id from contract c 
                        JOIN reference_contracts rc on  c.contract_Id = rc.contract_Id  and insurance_id = :insId
				 ) c on FIND_IN_SET(ml.PCP_PROVIDER_NBR, c.PCP_PROVIDER_NBR)
