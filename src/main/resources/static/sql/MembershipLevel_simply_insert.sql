 ...
 select  null,  :activityMonth,  :insId, c.prvdr_id, m.Mbr_Id,
...
PayToId	,PayToName	,ServLocAffiliation	,ServiceLocationId	,ServiceLocationName,:fileId, now(),now(),:username,:username,'Y'
  ...
  join membership m on m.SRC_SYS_MBR_NBR = ml.carriermemid
...
now() created_date,now() updated_date,:username created_by,:username updated_by 
   ...
   join membership m on m.SRC_SYS_MBR_NBR  =  csv2AmgMbrLvl.carriermemid  
...
sum(premium *.84) FUNDING, count(distinct carriermemid)MEMBER_MONTH_CNT, sum( ml.INST)INST_CLAIMS, sum( ml.PROF)PROF_CLAIMS, sum( ml.PHARM)PHAR_CLAIMS,
